...
class UIStructureTests(unittest.TestCase):
    ...
    def test_tray_menu_is_state_aware_and_fully_retranslated(self):
        ...
        toggle = Mock()
        memory_toggle = Mock()
        tray = SystemTrayManager(window, toggle_callback=toggle, model_memory_callback=memory_toggle)
        ...
        tray.set_dictation_state("idle")
        tray.set_model_memory_state("cuda", resident=True)
        self.assertTrue(tray.model_memory_action.isVisible())
        self.assertEqual(tray.model_memory_action.text(), "Oyun Modu — VRAM'i Boşalt")
        tray.model_memory_action.trigger()
        memory_toggle.assert_called_once_with()
        tray.set_model_memory_state("cuda", resident=False)
